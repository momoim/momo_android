...
public class ContactSyncManager {
    ...
    private void updateLocalContactsToServer() {
        if (syncManager.needStopSync)
            return;
        FlurryAgent.logEvent("updateLocalContactsToServer");
        List<Contact> momoContactsList = momoContactsManager.getAllContactsIdList();
    ...
    public boolean uploadLocalContactsToServer() {
        ...
        FlurryAgent.logEvent("uploadLocalContactsToServer");
        List<Contact> localContactsList = localContactsManager.getAllContactsList();
        try {
    ...
    private void updateMoMoContactsFromServer() {
        if (syncManager.needStopSync)
            return;
        FlurryAgent.logEvent("updateMoMoContactsFromServer");
        ...
        if (null != contactDetailsList && contactDetailsList.size() > 0) {
            if (syncManager.needSyncToLocal()) {
                ...
                if (null != resultList && resultList.size() > 0) {
                    ...
                    syncManager.sendMsg(SyncContactApi.MSG_SYNC_PROCESS_PART_FINISHED);
                    if (result)
                ...
                Log.d(TAG, "正要写入momo：" + contactDetailsList.size());
                boolean result = momoContactsManager.batchAddContacts(contactDetailsList, false);
                Log.d(TAG, "写入momo結果：" + result);
                syncManager.sendMsg(SyncContactApi.MSG_SYNC_PROCESS_PART_FINISHED);
                if (result)
                    batchInsertAvatar(contactDetailsList);
            }
        }
    }
    ...
    private void batchUpdateContactDetails(List<Contact> needUpdateList) {
        ...
        if (null != resultList && resultList.size() > 0) {
            ...
            if (result && syncManager.needSyncToLocal()) {
    ...
    /**
     * 根据MQ推送下来的联系人ID删除该联系人
     * 
     * @param contactId
     */
    public void mqDeleteContact(List<Long> contactIdList) {
        if (isToIgnoreMQ())
            return;
        ...
        try {
            ...
            Log.d(TAG, "mq delete size:" + contactIdList.size());
            if (syncManager.needSyncToLocal()) {
                List<Contact> localContactList = momoContactsManager
                        .getPhoneContactIdByContactId(contactIdList);
                deleteResult = localContactsManager.batchDeleteContacts(localContactList);
                localContactList.clear();
            }
            if (deleteResult) {
                momoContactsManager.batchDeleteContactByIdList(contactIdList);
            }
            Log.d(TAG, "mq delete end.");
        } catch (Exception ex) {
            Log.e(TAG, ex.toString());
        } finally {
            SyncManager.getInstance().setSyncInProgress(false);
            SyncManager.getInstance().sendMsg(SyncContactApi.MSG_MQ_PROCESS_FINISHED);
        }
    }
     ...
     * @param contactId
     */
    public void mqUpdateContact(List<Long> contactIdList, List<Long> modTimeList) {
        if (isToIgnoreMQ())
            return;
        if (null == contactIdList || null == modTimeList
                || contactIdList.size() != modTimeList.size())
            return;
        try {
            SyncManager.getInstance().setSyncInProgress(true);
            List<Contact> contactsList = new ArrayList<Contact>();
            for (int i = 0; i < contactIdList.size(); i++) {
                long contactId = contactIdList.get(i);
                Contact c = MoMoContactsManager.getInstance().getContactById(contactId);
                if (null == c || c.getModifyDate() == modTimeList.get(i)) {
                    continue;
                }
            ...
            Log.d(TAG, "mq update: " + count);
            if (count > 0) {
                batchUpdateToLocal(contactsList);
                contactsList.clear();
            }
            Log.d(TAG, "mq update end.");
        } catch (Exception ex) {
            Log.e(TAG, ex.toString());
        } finally {
            SyncManager.getInstance().setSyncInProgress(false);
     ...
     * @param contactId
     */
    public void mqAddContact(List<Long> contactIdList) {
        if (isToIgnoreMQ())
            return;
        Log.d(TAG, "mq add: " + contactIdList.size());
        try {
            SyncManager.getInstance().setSyncInProgress(true);
            List<Contact> contactsList = new ArrayList<Contact>();
            ...
            if (contactsList.size() > 0) {
                batchAddToLocal(contactsList);
                contactsList.clear();
            }
            Log.d(TAG, "mq add end.");
        } catch (Exception ex) {
            Log.e(TAG, ex.toString());
        } finally {
            SyncManager.getInstance().setSyncInProgress(false);
     ...
     * 判断是否忽略ｍｑ的推送
     */
    private boolean isToIgnoreMQ() {
        Context context = Utils.getContext();
        if (SyncManager.getInstance().isSyncInProgress())
            return true;
        String syncMod = GlobalUserInfo.getSyncMode(context);
        if (ConfigHelper.SYNC_MODE_LOCAL_ONLY.equalsIgnoreCase(syncMod)) {
            return true;
        } else if (ConfigHelper.SYNC_MODE_TWO_WAY.equalsIgnoreCase(syncMod)) {
            return false;
        } else {
            return true;
        }
    ...
    public void syncLocalToMoMo() {
        FlurryAgent.logEvent("syncLocalToMoMoWithoutUpload");
        List<Contact> momoContactsList = momoContactsManager.getAllContactsIdList();
        Collections.sort(momoContactsList, PhoneCIdComparator.getInstance());
        int momoSize = momoContactsList.size();
        Log.d(TAG, "all contacts on momo:" + momoSize);
     ...
     * @return
     */
    public boolean saveContact(String syncMode, Contact contact) {
        boolean result = false;
        if (contact == null) {
            return false;
        } else if (SyncManager.getInstance().isSyncInProgress()) {
            return false;
        }
        String net = Utils.getActiveNetWorkName(Utils.getContext());
        List<Contact> contactsList = new ArrayList<Contact>();
        contact.setContactId(0);
        Avatar avatar = contact.getAvatar();

        if ((contact.getFirstName() == null || contact.getFirstName().length() < 1)
                && (contact.getLastName() == null || contact.getLastName().length() < 1)) {
            contact.setFirstName(contact.getFormatName());
        }

        contactsList.add(contact);
        if (ConfigHelper.SYNC_MODE_TWO_WAY.equals(syncMode) && net != null) {
            try {
                saveContactsToServer(contactsList);
                Log.d(TAG, "save contact'card end.");
                result = true;
                if (contact.getUid() > 0) {
                    Map<String, Object> resultMap = CardManager.getInstance().retrieveUserCard(contact.getUid());
                    int code = Integer.valueOf(resultMap.get(CardManager.KEY_RESPONSE_STATUS).toString());
                    if(HttpStatus.SC_OK != code) {
                        result = false;
                    }
                }
                SyncContactApi.getInstance(Utils.getContext()).serverSync();
            } catch (Exception e) {
                e.printStackTrace();
                result = false;
            } finally {
            ...
            if (avatar != null && net != null) {
                byte[] image = downloadImageFromServer(avatar.getServerAvatarURL());
                avatar.setMomoAvatarImage(image);
            }

            List<Contact> resultList = localContactsManager.batchAddContacts(contactsList);
            if (null != resultList && resultList.size() > 0) {
                for (Contact localContact : resultList) {
                    localContact.setContactId(localContact.getPhoneCid());
                    localContact.setContactCRC(String.valueOf(localContact.generateCRC()));
                }
                Log.d(TAG, "正要写入momo：" + resultList.size());
                boolean localResult = momoContactsManager.batchAddContacts(resultList, true);
                Log.d(TAG, "写入momo結果：" + localResult);
                syncManager.sendMsg(SyncContactApi.MSG_SYNC_PROCESS_PART_FINISHED);
                if (localResult) {
                    batchInsertAvatar(resultList);
                }
