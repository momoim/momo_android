...
public class SlideModel extends Model implements List<MediaModel>, EventListener {
    ...
    private void internalAddOrReplace(MediaModel old, MediaModel media) {
        // If the media is resizable, at this point consider it to be zero
        // length.
        // Just before we send the slideshow, we take the remaining space in the
        // slideshow and equally allocate it to all the resizeable media items
        // and resize them.
    ...
    private boolean internalRemove(Object object) {
        if (mMedia.remove(object)) {
            ...
            // If the media is resizable, at this point consider it to be zero
            // length.
            // Just before we send the slideshow, we take the remaining space in
            // the
            // slideshow and equally allocate it to all the resizeable media
            // items and resize them.
    ...
    /**
     * Add a MediaModel to the slide. If the slide has already contained a media
     * object in the same type, the media object will be replaced by the new
     * one.
     * 
