...
import com.android.mms.http.HttpUtils;
import com.android.mms.util.SendingProgressTokenManager;

/**
 * Transaction is an abstract class for notification transaction, send
 * transaction and other transactions described in MMS spec. It provides the
 * interfaces of them and some common methods for them.
 */
public abstract class Transaction extends Observable {
    ...
    /**
     * An instance of Transaction encapsulates the actions required during a MMS
     * Client transaction.
     */
     ...
     * @return true if transaction is equivalent to this instance, false
     *         otherwise.
     */
    ...
    /**
     * Get the service-id of this transaction which was assigned by the
     * framework.
     * 
     ...
     * @throws IOException if any error occurred on network interface or an HTTP
     *             error code(>=400) returned from the server.
     */
     ...
     * @return A byte array which contains the data of the PDU. If the status
     *         code is not correct, an IOException will be thrown.
     * @throws IOException if any error occurred on network interface or an HTTP
     *             error code(>=400) returned from the server.
     */
    ...
    /**
     * Make sure that a network route exists to allow us to reach the host in
     * the supplied URL, and to the MMS proxy host as well, if a proxy is used.
     * 
        ...
        if (settings.isProxySet()) {
            ...
            if (inetAddr == -1) {
                ...
                if (!connMgr.requestRouteToHost(
                        /* ConnectivityManager.TYPE_MOBILE_MMS */2, inetAddr)) {
    ...
    /**
     * Look up a host name and return the result as an int. Works if the
     * argument is an IP address in dot notation. Obviously, this can only be
